...
async function createProduct(req, res) {
    try {
        ...
        const { productName, productDescription, productVarieties: [{ size, color, quantity, images, price }], dateUploaded, dateEdited } = JSON.parse(body);
...
async function removeProductById(req, res, id) {
    try {
        const product = await Product.findById(id);
        if (!product) {
            res.writeHead(404, headers);
